...
import { supabase } from "@/lib/supabase"; // 忘れずに！
...
export default function CreateNews() {
  ...
  // 💡 【新規追加】画像をアップロードしてMarkdownに挿入する関数
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // ファイル名が被らないように、現在時刻の数字をファイル名の前につける
    const fileName = `${Date.now()}-${file.name}`;

    alert("画像をアップロード中です...");

    try {
      // 1. Supabaseの 'images' バケットに画像をアップロード
      ...
      // 3. 今書いている本文（content）の末尾に、Markdown形式で画像URLを追記する
      setFormData(prev => ({
        ...prev,
